...
from ripe.atlas.cousteau import (Ping, Traceroute, Dns, Sslcert, Http, Ntp,
                                 AtlasSource, AtlasCreateRequest)
...
class Command(BaseCommand):
    ...
    CREATION_CLASSES = OrderedDict(
        (("ping", Ping), ("traceroute", Traceroute), ("dns", Dns),
         ("sslcert", Sslcert), ("http", Http), ("ntp", Ntp)))
    ...
    def _modify_parser_args(self, args):
        ...
        error = ("Usage: ripe-atlas measure <{}> [options]\n"
                 "\n"
    ...
    def run(self):
        ...
        # LC: this print is necessary to retrieve the measurement id
        # of the newly created measurement. In commands.base there is
        # a method definition, namely `ok`, that print to standard
        # output only if the ripe-atlas tool is invoked by
        # terminal. Despite `mucera` calls ripe-atlas from terminal,
        # it is not the terminal and therefore nothing is printed to
        # stdout.
        print(pk)
        ...
        for param, val in self._get_source_kwargs().items():
            if param == "tags":
                print(
                    colourise(
                        "tags\n  include{}{}\n  exclude{}{}\n".format(
                            " " * 17, ", ".join(val["include"]), " " * 17,
                            ", ".join(val["exclude"])), "cyan"))
                continue
            print(colourise("{:<25} {}".format(param, val), "cyan"))

    def create(self):
        creation_class = self.CREATION_CLASSES[self._type]
        # print("LC: Am I called create?")
    ...
    def stream(self, pk, url):
        ...
        try:
            Stream(capture_limit=self.arguments.probes,
                   timeout=300).stream(self.arguments.renderer, self.arguments,
                                       self._type, pk)
    ...
    def clean_target(self):
        # LC: Added cloud endpoints
        if not self.arguments.target:
            raise RipeAtlasToolsException(
                "You must specify a target for that kind of measurement")

        # if self.arguments.target else self.arguments.cloud_endpoints
        return self.arguments.target  # .split(",")
    ...
    def clean_description(self):
        ...
        return "{} measurement to {}".format(self._type.capitalize(),
    ...
    def _get_measurement_kwargs_lc(self):
